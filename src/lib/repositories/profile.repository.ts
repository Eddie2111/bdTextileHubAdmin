...
export async function getOneUserWithProfile({ id }: { id: string }) {
  try {
    ...
    const wishlistProductIds = oneUserProfile.wishlists.flatMap(
      wishlist => wishlist.productId,
    );
    const orderProductIds = oneUserProfile.orders.flatMap(
    ...
    const allProductIds = Array.from(
      new Set([...wishlistProductIds, ...orderProductIds, ...cartProductIds]),
    );
